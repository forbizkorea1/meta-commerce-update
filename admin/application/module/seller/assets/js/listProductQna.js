 ...
 * 개발 *
 *--------------------------------------------------------------*/
var devSellerListProductQnaObj = {
    grid: false,
    initLang: function () {
    ...
    initForm: function () {
        ...
        common.form.init(
                $('#devHidePQ'),
                common.util.getControllerUrl('putProductQna', 'listProductQna', 'seller'),
                function (formData) {
                    return true;
                },
                function (response) {
                    if (response.result == 'success') {
                        if ($("[name=devHideModePQ]").val() == "delete") {
    ...
    initPagingGrid: function () {
        ...
        var gridConfig = {
            ...
            columns: [
                ...
                {key: "com_name", label: common.lang.get('grid.label.com_name'), width: 150, align: "left"},
                {key: "bbs_re_cnt", label: common.lang.get('grid.label.bbs_re_cnt'), width: 150, align: "center", formatter: function () {
        ...
        self.grid.setGrid($('#devPagingGrid'), gridConfig)
                ...
                .setUseHash(false)
                .setUrl(common.util.getControllerUrl('getProductQnaList', 'listProductQna', 'seller'))
        ...
        $('#devPagingGrid [data-ax5grid-panel="body"]').on('click', '.devGridDataModify', function () {
            ...
            common.form.init(
                    $('#devHideCmt'),
                    common.util.getControllerUrl('put', 'listProductQna', 'seller'),
                    function (formData) {
                        return true;
                    },
                    function (response) {
                        if (response.result == 'success') {
                            if ($("#devHideMode").val() == "update") {
                                common.noti.alert(common.lang.get('common.put.success.alert'));
                            } else if ($("#devHideMode").val() == "insert") {
...
//                    .init();
        ...
        $('#devPagingGrid [data-ax5grid-panel="body"]').on('click', '.devGridDataDel', function () {
            ...
            if (common.noti.confirm(common.lang.get('common.del.confirm', {title: row.pname}))) {
                common.ajax(common.util.getControllerUrl('delListProductQna', 'listProductQna', 'seller'),
                        {bbs_ix: row.bbs_ix},
                        function () {
                            // 전송전 데이타 검증
                            return true;
    ...
    getAjaxComentList: function () {
        var self = this;
        common.ajax(
                common.util.getControllerUrl('getProductQnaListComent', 'listProductQna', 'seller'),
                {
                    bbs_ix: self.bbs_ix
                },
                function () {
...
$(function () {
    devSellerListProductQnaObj.run();
});