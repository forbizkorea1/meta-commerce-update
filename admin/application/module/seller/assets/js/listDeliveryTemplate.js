 ...
 * 개발 *
 *--------------------------------------------------------------*/
var devSellerListDeliveryTemplateObj = {
    initLang: function () {
        // 글로벌 언어셋 설정
        ...
        common.lang.load('validation.rep.check', '배송정책 템플릿 선택해주세요.');
    ...
    initForm: function () {
        ...
        $(".devSave").on('click', function () {
            common.form.init(
                    $(self.devTap),
                    common.util.getControllerUrl(self.method, 'listDeliveryTemplate', 'seller'),
                    function (formData) {
    ...
    initListDeliveryTemplateGrid: function () {
        ...
        $('#devPagingGrid [data-ax5grid-panel="body"]').on('click', '.devGridDataModify', function () {
            ...
            document.location.href = '/seller/manageDeliveryTemplate/' + row.dt_ix;
        });
    ...
    tapGroupDeliveryGrid: function () {
        ...
        self.groupDeliveryGrid.setGrid($('#devPagingGorupGrid'), gridConfigGroup)
                ...
                .setUrl(common.util.getControllerUrl('getGroupDelivery', 'listDeliveryTemplate', 'seller'))
                .init(function (response) {
        ...
        $('#devPagingGorupGrid [data-ax5grid-panel="body"]').on('click', '.devGridDataModify', function () {
            ...
            if (row.state == 1) {
                $("#devState1").prop("checked", true);
            } else {
                $("#devState0").prop("checked", true);
            }

            $(".addTemplateTbody tr").remove();
            common.ajax(
                    common.util.getControllerUrl('getGroupContensList', 'listDeliveryTemplate', 'seller')
                    , {g_ix: row.g_ix}, false
                    , function (response) {
                        if (response.result == 'success') {
        ...
        $('#devPagingGorupGrid [data-ax5grid-panel="body"]').on('click', '.devGridDataDel', function () {
            ...
            if (common.noti.confirm(common.lang.get('common.del.confirm', {title: row.name}))) {
                common.ajax(common.util.getControllerUrl('delGroupList', 'listDeliveryTemplate', 'seller'),
                        {g_ix: row.g_ix,
...
$(function () {
    devSellerListDeliveryTemplateObj.run();
});