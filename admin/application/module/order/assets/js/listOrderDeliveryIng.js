 ...
 * 개발 *
 *--------------------------------------------------------------*/
var devOrderListOrderDeliveryIngObj = {
    initLang: function () {
        // 글로벌 언어셋 설정
    ...
    initGrid: function () {
        ...
        self.grid.setGrid($('#devPagingGrid'), gridConfig)
            ...
            .setUrl(common.util.getControllerUrl('get', 'listOrderDeliveryIng', 'order'))
            .on('click', function (e) {
                if (e.column.key == 'oid') {
    ...
    initForm: function () {
        var self = this;
        common.form.init($('#devModifyForm'), common.util.getControllerUrl('put', 'listOrderDeliveryIng', 'order'), function (formData, $form) {
            ...
            if (coOdIxList.length > 0 && formData[0].value == 'DC' && orderFromList == 'self') {
...
$(function () {
    devOrderListOrderDeliveryIngObj.run();
});