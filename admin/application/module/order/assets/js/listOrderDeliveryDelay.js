 ...
 * 개발 *
 *--------------------------------------------------------------*/
var devOrderListOrderDeliveryDelayObj = {
    initLang: function () {
        // 글로벌 언어셋 설정
    ...
    initGrid: function () {
        ...
        self.grid.setGrid($('#devPagingGrid'), gridConfig)
            ...
            .setUrl(common.util.getControllerUrl('get', 'listOrderDeliveryDelay', 'order'))
            .on('click', function (e) {
                if (e.column.key == 'oid') {
    ...
    initForm: function () {
        var self = this;
        common.form.init($('#devModifyForm'), common.util.getControllerUrl('put', 'listOrderDeliveryDelay', 'order'), function (formData, $form) {
...
$(function () {
    devOrderListOrderDeliveryDelayObj.run();
});