 ...
 * 개발 *
 *--------------------------------------------------------------*/
var devOrderListOrderReturnObj = {
    initLang: function () {
        // 글로벌 언어셋 설정
        ...
        common.lang.load('grid.label.ra_date', '반품요청일자');
        common.lang.load('grid.label.claimReason', '반품사유');
        ...
        common.lang.load('common.npay.rm_fail.alert', '반품불가는 네이버페이센터에서 관리해주세요.');
    },
    initTab: function () {
        var self = this;
        common.ui.tap($('#devTap'), function (selector) {
            ...
            if (status == 'RC') {
                $('#devModifySection').css("display", "none");
            } else {
                $('#devModifySection').css("display", "");
            }
    ...
    initGrid: function () {
        ...
        var gridConfig = {
            ...
            columns: self.getAuthGridColumns([
                ...
                {key: "ra_date", label: common.lang.get('grid.label.ra_date'), width: 130, align: 'center'},
    ...
    initForm: function () {
        var self = this;
        common.form.init($('.devModifyForm'), common.util.getControllerUrl('put', 'listOrderReturn', 'order'), function (formData, $form) {
            ...
            if (coOdIxList.length > 0 && orderFromList == 'self') {
                if (formData[0].value == 'RM') {
                    common.noti.alert(common.lang.get('common.npay.rm_fail.alert'));
                    return false;
                }
            }
...
$(function () {
    devOrderListOrderReturnObj.run();
});