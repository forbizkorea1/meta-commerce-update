 ...
 * 개발 *
 *--------------------------------------------------------------*/
var devProductBatchChangeCommissionObj = {
    initLang: function () {
        // 글로벌 언어셋 설정
    ...
    initPagingGrid: function () {
        ...
        var gridConfig = {
            ...
            columns: [
                ...
                {
                    key: "commissionText",
                    label: common.lang.get('grid.label.commissionText'),
                    width: 80,
                    align: 'center'
                },
        ...
        self.listGrid.setGrid($('#devPagingGrid'), gridConfig)
            ...
            .setUrl(common.util.getControllerUrl('get', 'batchChangeCommission', 'product'))
        ...
        $('#devExcelDownload').on('click', function (e) {
            grid.excelDown(common.util.getControllerUrl('dwn', 'batchChangeCommission', 'product'));
        });
    },
    initBatch: function () {
        ...
        $("input:radio[name=one_commissionBatch]").click(function () {
            if ($("input[name=one_commissionBatch]:checked").val() == "Y") {
                $('#commissionTr1').show();
                $('#commissionTr2').show();
            } else {
                $('#commissionTr1').hide();
    ...
    initForm: function () {
        ...
        // 입력/수정폼 설정
        common.form.init($form, common.util.getControllerUrl('put', 'batchChangeCommission', 'product'),
            function (formData) {
                var ids = devProductBatchChangeCommissionObj.listGrid.getList('selected', ['id']);
                if ($('input[name=one_commissionBatch]').val() == 'Y' && $('input[name=commissionBatch]').val() == false) {
                    $('input[name=commissionBatch]').focus();
...
$(function () {
    devProductBatchChangeCommissionObj.run();
