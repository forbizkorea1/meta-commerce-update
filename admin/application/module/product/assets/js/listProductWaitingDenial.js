 ...
 * 개발 *
 *--------------------------------------------------------------*/
var devProductListProductWaitingDenialObj = {
    initLang: function () {
        // 글로벌 언어셋 설정
    ...
    initPagingGrid: function () {
        ...
        grid.setGrid($('#devPagingGrid'), gridConfig)
            ...
            .setUseHash(false)
            .setUrl(common.util.getControllerUrl('get', 'listProductWaitingDenial', 'product'))
        ...
        $('#devExcelDownload').on('click', function (e) {
            grid.excelDown(common.util.getControllerUrl('dwn', 'listProductWaitingDenial', 'product'));
        });
    },
    initForm: function () {
        //상품 검색시 다중검색 처리
...
$(function () {
    devProductListProductWaitingDenialObj.run();
});