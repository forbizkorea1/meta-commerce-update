...
var devMemberBatchGiveCouponObj = {
    initLang: function () {
        ...
        common.lang.load('common.add.confirm.alert', '선택한 회원에게 쿠폰을 일괄 지급 하시겠습니까?');
        common.lang.load('common.add.success.alert', '저장이 완료되었습니다.');
        common.lang.load('common.put.success.alert', '수정이 완료되었습니다.');
        common.lang.load('common.give.cupon.alert', '쿠폰 일괄 지급이 완료 되었습니다.');
        common.lang.load('common.put.error.alert', '쿠폰 일괄 지급이 실패 되었습니다.');
        ...
        common.form.init(
            ...
            function (formData) {
                ...
                if (common.validation.check($('#devModifyForm'), 'alert', false)) {
                    if (common.noti.confirm(common.lang.get('common.add.confirm.alert'))) {
                        return formData;
                    } else {
                        return false;
                    }
    ...
    initEvent: function () {
        ...
        $('#devCouponType').on('change', function () {
            if ($(this).val() == 'G')
            {
                $('#devUpdateCartCouponList').hide();
                $('#devUpdateProductCouponList').show();

                document.getElementById('devUpdateProductCouponList').setAttribute('name', 'update_ix');
                document.getElementById('devUpdateCartCouponList').setAttribute('name', 'update_ix_02');

                common.validation.set($('#devUpdateCartCouponList'), {'required': false});
                common.validation.set($('#devUpdateProductCouponList'), {'required': true});
            } else if ($(this).val() == 'C') {
                $('#devUpdateProductCouponList').hide();
                $('#devUpdateCartCouponList').show();

                document.getElementById('devUpdateProductCouponList').setAttribute('name', 'update_ix_01');
                document.getElementById('devUpdateCartCouponList').setAttribute('name', 'update_ix');

                common.validation.set($('#devUpdateProductCouponList'), {'required': false});
                common.validation.set($('#devUpdateCartCouponList'), {'required': true});
            } else {
                $('#devUpdateProductCouponList').hide();
                $('#devUpdateCartCouponList').hide();
    ...
    initTab: function () {
        var self = devMemberBatchGiveCouponObj;
    ...
    initPagingGrid: function () {
        ...
        var gridConfig = {
            ...
            columns: [
                {key: "gp_name", label: common.lang.get('grid.label.gp_name'), width: 100, align: "center"},
                ...
                {key: "pcs", label: common.lang.get('grid.label.pcs'), width: 100, align: "center"},
                {key: "mail", label: common.lang.get('grid.label.mail'), width: 200, align: "center"},
                ...
                {key: "act", label: common.lang.get('grid.label.act'), align: "center", width: 100, formatter: function () {
                        return [
    ...
    checkValidation: function () {
        ...
        common.validation.set($('[name=coupon_type]'), {'required': true});
    },
