...
var devMiniCart = function () {
    return {
        ...
        _makeOptionsArea: function () { //옵션 셀렉트박스 생성
            ...
            $(this.lonelyContents).show(); //2021.07.28 화면변경됨. 단일옵션 해당영역 미사용. 복수옵션 옵션선택 텍스트 노출.
            $(this.lonelyContents).append(this.lonelyTemplate(""));

            // str.push('<span class="opt-title">옵션선택</span>');
        ...
        _checkSameOption: function (id) { //선택된 옵션이 이미 추가되어있는지 확인
            var check = $('[devOptid="' + id + '"]').length;
            if (check == 0) {
                return true;
            } else {
                $(' [devOptid='+id+']').find(".js__sameOption__plus").trigger("click");
                return false;
            }
        },
        ...
        _cancelChoosedEventBind: function () { //선택된 옵션 삭제
            ...
            $(self.choosedContents).on('click', self.deleteBtn, function () {
                ...
                if (originalProduct == 0) { //본 상품 개수 체크
        ...
        _checkCountValidation: function (type, cnt, stock,optionKind) { //주문개수 제한 조건 확인
            ...
            if (type == 'up') {
                ...
                if (parseInt(cnt) - 1 >= allowBasicCnt) { //최소구매숫자 이상
                    data.cnt = parseInt(cnt) - 1;
                    return data;
                } else {
        ...
        _add: function (type) { //type : 바로구매 D / 장바구니 C
            ...
            if ($(self.choosedDetails).length == 0) {
                ...
                var data = []; //본품
                var addData = []; //추가구성
                $(self.choosedDetails).each(function () { //각 옵션박스 영역 루프
                    if ($(this).attr('devOptionkind') != 'a') { //추가구성 외 일반 옵션 데이터 생성
                        var dataComponent = {
                            pid: pid,
