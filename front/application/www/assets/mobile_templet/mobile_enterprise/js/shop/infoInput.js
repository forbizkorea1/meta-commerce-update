...
common.validation.set($('#devBuyerEmail'), { 'required': true, 'dataFormat': 'email', 'getValueFunction': 'devInfoinputObj.getBuyerEmail' });
...
var devInfoinputObj = {
    ...
    changeOrderData: function() {
        ...
        common.ajax(common.util.getControllerUrl('getChangeOrderData', 'order'), requestData, (function() {}), (function(response) {
            if (response.result == 'success') {
    ...
    getPaymentData: function() {
        var self = this;
        var data = {};
        data.method = $('.on[devPaymentMethod]').attr('devPaymentMethod');
    ...
    memberRun: function() {
        ...
        $('#devAddressListButton').on('click', function() {
            ...
            common.util.modal.open('ajax', '배송지 선택', '/mypage/addressbookSelect/'+ix, '', function() {
                devAddressBookPopObj.callbackSelect = function(deliveryIx) {
                    common.ajax(
                        common.util.getControllerUrl('getAddressBook', 'order'), { deliveryIx: deliveryIx },
                        function() {
                            return deliveryIx;
                        },
                        function(response) {
        ...
        $('#devUseCouponButton').click(function() {
            common.util.modal.open('ajax', '쿠폰변경', '/shop/couponPop', { cartIxs: self.getCartIx(), useCouponData: self.useCouponData }, function () {
                $(".fb__popup-layout").addClass('full-layer floating-btn couponPopup');
            });
        });
